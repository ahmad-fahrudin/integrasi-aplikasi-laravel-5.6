...
class BarangController extends Controller
{
  ...
  public function index(){
    if (role() && (Auth::user()->gudang == "1" || Auth::user()->gudang == "2")) {
  ...
  public function barangmasuk(Request $post){
    ...
    if ($data['date_from'] != null) { $from = $data['date_from']; }
    if ($data['date_to'] != null) { $to = $data['date_to']; }
    if ($data['driver'] != null) { $dt['driver'] = $data['driver']; $data['id_driver'] = $data['driver']; }
    if ($data['suplayer'] != null) { $dt['suplayer'] = $data['suplayer']; $data['id_suplayer'] = $data['suplayer']; }
    if ($data['qc'] != null) { $dt['qc'] = $data['qc']; $data['id_qc'] = $data['qc']; }
    if(isset($from) && isset($to) && isset($dt)){
      ...
      ->select("a.id","g.nama_gudang","a.no_faktur","a.tgl_masuk","b.alamat","b.nama_pemilik","c.no_sku","c.nama_barang","a.jumlah","d.nama as driver","e.nama as qc","f.name as admin")
      ->where($dt)
  ...
  public function inputbarangmasuk(){
    if (role() && (Auth::user()->gudang == "1" || Auth::user()->gudang == "2" || Auth::user()->gudang == "9")) {
      $data['suplayer'] = DB::table('tb_suplayer as a')->select("*")->where("a.status","=","aktif")->get();
      $data['qc'] = DB::table('tb_karyawan as a')->select("*")->where("a.status","=","aktif")->get();
      $data['driver'] = DB::table('tb_karyawan as a')->select("*")->where("a.status","=","aktif")->get();
  ...
  public function editBarangMasuk($id){
    ...
    $data['barang'] = DB::table('tb_barang_masuk as a')
      ...
      ->select("a.id","a.gudang","a.barang","b.nama_pemilik as suplayer","a.jumlah","d.nama as driver","f.name as admin","e.nama as qc",
              "a.suplayer as id_suplayer","a.driver as id_driver","a.admin as id_admin","a.qc as id_qc")
      ->where("a.status","=","aktif")
      ->where("a.id","=",$id)
      ->get();
  ...
  public function stokgudang(){
    if (role()) {
      ...
      ->select("a.*","a.id as id_barang","c.harga",DB::raw('SUM(b.jumlah) as stok'))
      ...
      foreach ($data['stok'] as $key => $value) {
        ...
        $data['asset'] += $value->harga * $value->stok;
      }
  ...
  public function stokgudangs(Request $post){
    if (role()) {
      ...
      if(isset($d['id_gudang']) && $d['id_gudang'] != "all"){ $x['id_gudang'] = $d['id_gudang']; }
      if(isset($d['jumlah'])){ $s = $d['jumlah']; $data['s'] = $d['jumlah']; }
      if(isset($d['lokasi'])){ $x['lokasi'] = $d['lokasi']; $data['x'] = $d['lokasi']; }
      if(isset($d['stok'])){
        if ($d['stok'] == "stok") {
          $u = "0";
        }else{
  ...
  public function postbarangmasuk(Request $post){
    $data = $post->except('no_faktur','_token','id_gudang','barang','jumlah');
    ...
    for ($i=1; $i < count($split); $i++) {
      ...
      $harga = DB::table('tb_harga as a')->select("harga")->where("a.id_barang","=",$dt['id_barang'])->get();
  ...
  public function daftarorderstok(){
    if (role()) {
      ...
      $q = 'DELETE tb_transfer_stok , tb_detail_transfer FROM tb_transfer_stok INNER JOIN tb_detail_transfer on
      tb_transfer_stok.no_transfer = tb_detail_transfer.no_transfer where tb_transfer_stok.tanggal_order < "'.$del.'" AND tb_transfer_stok.status_transfer="order"';
      DB::delete($q);
...
public function verifikasipenjualan(){
  ...
  public function inputorderbaru(){
    if (role()) {
      ...
      $data['barang'] = DB::table('tb_barang as a')->join('tb_harga as b','b.id_barang','=','a.id')
                        ->select("a.*","harga","harga_hp","qty1","pot1","qty2","pot2","qty3","pot3")->where("a.status","=","aktif")->get();
  ...
  public function cariKaryawan($id,$status){
    $p = DB::table('tb_karyawan as a')->select("a.id")->where("a.nik","=",Auth::user()->nik)->get();
    if (Auth::user()->gudang == "1" || Auth::user()->gudang == "2") {
  ...
  public function daftarorderbaru(){
    if (role()) {
      ...
      $q = 'DELETE tb_barang_keluar , tb_detail_barang_keluar FROM tb_barang_keluar INNER JOIN tb_detail_barang_keluar on tb_barang_keluar.no_kwitansi = tb_detail_barang_keluar.no_kwitansi where tb_barang_keluar.tanggal_order < "'.$del.'" AND tb_barang_keluar.status_barang="order"';
      DB::delete($q);
      ...
      if (Auth::user()->level == "5") {
        ...
        $data['order'] = DB::table('tb_barang_keluar as a')
                         ...
                         ->select("a.id","a.no_kwitansi","a.tanggal_order","b.nama_pemilik","b.alamat","b.kota","c.nama as sales",
                                  "d.name as admin_p","a.admin_p as id_admin_p","e.nama_gudang","f.nama_status","a.status_barang","a.ket")
        ...
        $data['lain'] = DB::table('tb_barang_keluar as a')
                         ...
                         ->join('tb_gudang as e','e.id','=','a.id_gudang')
                         ->join('tb_status_order as f','f.id','=','a.status_order')
                         ->select("a.id","a.no_kwitansi","a.tanggal_order","b.nama","b.alamat","c.nama as sales",
                                  "d.name as admin_p","a.admin_p as id_admin_p","e.nama_gudang","f.nama_status","a.status_barang","a.ket")
                         ->where("a.status","=","aktif")->where("a.status_barang","=","order")->where("a.id_gudang","=",Auth::user()->gudang)->orWhere("a.status_barang","=","kirim ulang")->where("a.id_gudang","=",Auth::user()->gudang)->orderBy("a.tanggal_order","ASC")->get();
  ...
  public function daftarorderbarus(Request $post){
    if (role()) {
      ...
      if($d['status_order'] != null){ $u['a.status_order'] = $d['status_order']; $data['v_status_order'] = $d['status_order']; }
      if($d['sales'] != null){ $u['a.sales']=$d['sales']; $data['v_sales'] = $d['sales']; }
      if($d['pengembang'] != null){ $u['a.pengembang']=$d['pengembang']; $data['v_pengembang'] = $d['pengembang']; }
      if($d['id_gudang'] != null){ $u['a.id_gudang']=$d['id_gudang']; $data['id_gudang'] = $d['id_gudang']; }
      if($d['kota'] != null){ $u['b.kota']=$d['kota']; $data['kota'] = $d['kota']; }
  ...
  public function dataorderpenjualans(Request $post){
    if(role()){
      $d = $post->except('_token');
      if($d['from'] != null && $d['to'] != null){ $from=$d['from']; $to=$d['to']; $data['from'] = $d['from']; $data['to'] = $d['to']; }
      if($d['status_order'] != null){ $u['status_order']=$d['status_order']; $data['v_status_order'] = $d['status_order']; }
      if($d['id_gudang'] != null){ $u['id_gudang']=$d['id_gudang']; $data['id_gudang'] = $d['id_gudang']; }
      if($d['id_konsumen'] != null){ $u['id_konsumen']=$d['id_konsumen']; $data['id_konsumen'] = $d['id_konsumen']; $data['name_konsumen'] = $d['name_konsumen']; }
      if($d['status_barang'] != null){ $u['status_barang']=$d['status_barang']; $data['status_barang'] = $d['status_barang']; }
      if($d['branded'] != ""){ /*$u['branded']=$d['branded'];*/ $data['branded'] = $d['branded']; }
      if($d['petugas'] != null && $d['id'] != null){ $u[$d['petugas']] = $d['id']; $data['petugas'] = $d['petugas']; $data['id'] = $d['id']; }
      if($d['proses'] != null){
    ...
    if(role()){
      if (Auth::user()->level == "1") {
        ...
        $data['transfer2'] = DB::table('tb_barang_keluar as a')
                            ...
                            ->join('tb_karyawan as g','g.id','=','a.manager')
                            ->join('users as h','h.id','=','a.admin_p')
                            ->select("a.id","a.no_kwitansi","b.kategori","a.tanggal_order","b.id as id_konsumen","b.nama as nama_pemilik","b.alamat","b.no_hp"
                                     ,"c.nama_gudang","d.nama as sales","e.nama as pengembang","f.nama as leader","g.nama as manager","h.name as admin_p")
                            ->where("a.status_barang","=","order")->get();
      }else{
        $data['transfer'] = DB::table('tb_barang_keluar as a')
                            ->join('tb_konsumen as b','b.id','=','a.id_konsumen')
  ...
  public function updatebarangkeluar(Request $post){
    ...
    DB::table('tb_barang_keluar')->where('no_kwitansi','=',$d['no_kwitansi'])->increment('total_bayar',  $d['ongkos_kirim']);
    ...
    if (role()) {
      if (Auth::user()->level == "1") {
        ...
        $data['transfer2'] = DB::table('tb_barang_keluar as a')
                            ...
                            ->join('users as h','h.id','=','a.admin_p')
                            ->join('users as m','m.id','=','a.admin_g')
                            ->select("a.id","a.no_kwitansi","a.tanggal_order","b.id as id_konsumen","b.nama as nama_pemilik","b.alamat","b.no_hp"
                                     ,"c.nama_gudang","d.nama as sales","e.nama as pengembang","f.nama as leader","g.nama as manager"
                                     ,"j.nama as qc","a.dropper as id_dropper","k.nama as dropper","a.pengirim as id_pengirim","a.helper as id_helper","l.nama as pengirim","m.name as admin_g","a.tanggal_proses"
                                     ,"h.name as admin_p","b.kategori","s.nama as helper","a.ongkos_kirim")
                            ->where("a.status_barang","=","proses")
                            ->orWhere("a.status_barang","=", 'kirim ulang')->get();
      }else{
        $data['transfer'] = DB::table('tb_barang_keluar as a')
  ...
  public function penerimaanbarangkeluar(Request $post){
    ...
    foreach ($get as $value) {
      ...
      if($value->return > 0){
        ...
        //DB::table('tb_gudang_barang')->where('id_barang','=',$value->id_barang)->where('id_gudang','=',$value->id_gudang)->increment('jumlah', $value->return);
      }else{
        DB::table('tb_detail_barang_keluar')->where('id','=',$value->id)->update($s);
      }
    ...
    if(role()){
      ...
      if(Auth::user()->level == "5"){
        ...
        $data['penjualan'] = DB::table('tb_barang_keluar as a')
                            //->join('tb_konsumen as b','b.id','=','a.id_konsumen')
                            //->join('tb_karyawan as c','c.id','=','a.sales')
                            //->join('tb_karyawan as d','d.id',"=",'a.dropper')
                            //->join('tb_karyawan as e','e.id',"=",'a.pengirim')
                            //->join('users as f','f.id','=','a.admin_p')
                            //->join('users as g','g.id','=','a.admin_g')
                            //->join('users as h','h.id','=','a.admin_v')
                            //->leftJoin('tb_pembayaran as i', 'i.no_kwitansi', '=', 'a.no_kwitansi')
                            ->select("a.*")
                            ->where("a.pengembang","=",$p[0]->id)
    ...
    if(role()){
      ...
      if (isset($from) && isset($u) && isset($tempo)) {
        $data['penjualan'] = DB::table('tb_barang_keluar as a')
                            ...
                            ->join('users as h','h.id','=','a.admin_v')
                            ->leftJoin('tb_pembayaran as i', 'i.no_kwitansi', '=', 'a.no_kwitansi')
                            ->select("a.id","a.no_kwitansi","a.tanggal_terkirim","b.nama_pemilik","b.alamat","c.nama as sales","d.nama as dropper","e.nama as pengirim",
                                      "f.name as admin_p","g.name as admin_g","h.name as admin_v")
                            ...
                            ->orWhere("i.status_pembayaran","=","Titip")
                            ->whereBetween('a.tanggal_terkirim',[$from,$to])
                            ->where($u)
                            ->whereDate('a.tanggal_terkirim','<',$tempo)
                            ->where("a.status_barang","=","terkirim")->get();
  ...
  public function prosespembayaran(){
    if(role()){
      if (Auth::user()->level == "1") {
        $data['transfer'] = DB::table('tb_barang_keluar as a')
                            //->leftJoin('tb_pembayaran as o','a.no_kwitansi','=','o.no_kwitansi')
                            ->select("*","a.no_kwitansi as no_kwitansi","a.id as id")
                            ->where("a.status_barang","=","terkirim")
                            ->where("a.status_order","=","1")
                            ->where("a.total_bayar",">",0)
                            //->whereNull("o.status_pembayaran")
                            //->orWhere("o.status_pembayaran","Titip")
                            //->where("a.status_barang","=","terkirim")
                            //->where("a.status_order","=","1")
                            ->get();
      }else{
    ...
    if(role()){
      ...
      if (Auth::user()->level == "5") {
        ...
        /*$data['pembayaran'] = DB::table('tb_barang_keluar as a')
                            ...
                            ->get();*/
      }else if(Auth::user()->gudang == "2" || Auth::user()->gudang == "1"){
        $data['pembayaran'] = array();
        /*$data['pembayaran'] = DB::table('tb_barang_keluar as a')
                    ->select("*")
                    ->where("a.status_barang","terkirim")
  ...
  public function datapembayarans(Request $post){
    if(role()){
      ...
      if($k['id_gudang'] != null){ $u['a.id_gudang'] = $k['id_gudang']; $data['id_gudang'] = $k['id_gudang'];}
      ...
      if($k['status_order'] != null){ $u['a.status_order'] = $k['status_order']; $data['v_status_order'] = $k['status_order']; }
