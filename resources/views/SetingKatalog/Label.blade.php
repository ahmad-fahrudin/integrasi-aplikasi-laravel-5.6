...
@section('content')
                  ...
                  <div class="card-body">
                    <h4 class="card-title">
                      <nav aria-label="breadcrumb">
                          <ol class="breadcrumb ">
                              <li class="breadcrumb-item text-muted" aria-current="page">Katalog Produk > <a href="https://stokis.app/?s=tambah+label+produk" target="_blank">Label Produk</a></li>
                          </ol>
                      </nav>
                    </h4>
                    <hr>
                    ...
                    <a hidden href="{{url('label')}}" target="_blank" class="btn text-warning"><i data-feather="tag" class="feather-icon"></i> Label</a>
                    ...
                    <button type="button" class="btn btn-info btn-md" data-toggle="modal" data-target="#myModal">Tambah Label Baru</button>
                    <hr>
                  </div>
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <center><h3>Tambah Label</h3></center>
            <form action="{{url('addlabel')}}" method="post" enctype="multipart/form-data">
            {{csrf_field()}}
            Nama Label:
            <input required name="nama" type="text" required class="form-control" placeholder="Ketik nama label baru...">
            <br>
            Class:
            <input required name="class" type="text" required class="form-control" placeholder="Ketik success (hijau) atau danger (merah)...">
            <br>
    ...
    <div class="modal fade" id="editmodal" role="dialog">
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <center><h3>Ubah Label</h3></center>
            <form action="{{url('updatelabel')}}" method="post" enctype="multipart/form-data">
            {{csrf_field()}}
            <input type="hidden" name="id" id="id">
            Nama Label:
            <input required id="nama_jabatan" name="nama" type="text" class="form-control">
            <br>
            Class:
            <input required id="class" name="class" type="text" class="form-control">
            <br>
    ...
    function edit(value)
    {
        $.ajax({
           ...
           success: function(response){
             ...
             document.getElementById("nama_jabatan").value = response[0]['nama'];
             document.getElementById("class").value = response[0]['class'];
    ...
    function Deleted(name,value)
    {
      Swal.fire(
        ...
        if (result.value) {
          location.href="{{url('/deletelabel/')}}"+"/"+value;
        }
      });
    }
    </script>
