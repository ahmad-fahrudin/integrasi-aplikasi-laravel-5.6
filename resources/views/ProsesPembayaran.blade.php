...
@section('content')
                  ...
                  <div class="card-body">
                    <h4 class="card-title">
                      <nav aria-label="breadcrumb">
                          <ol class="breadcrumb ">
                              <li class="breadcrumb-item text-muted" aria-current="page">Keuangan > Piutang > Penjualan Produk > <a href="https://stokis.app/?s=proses+bayar+piutang+penjualan" target="_blank">Proses Bayar Piutang</a></li>
                          </ol>
                      </nav>
                    </h4>
                    <hr>
                            ...
                            <div class="row">
                                <div class="col-md-11">
                                  <div class="input-group">
                                    ...
                                    <input id="no_kwitansi" type="text" class="form-control" placeholder="Pilih No. Kwitansi Penjualan Terkirim">
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                    <div class="input-group">
                                      <input id="leader" type="text" class="form-control" readonly>
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                    <div class="input-group">
                                      <input id="manager" type="text" class="form-control" readonly>
                        ...
                        <div hidden class="row">
                          ...
                          <div class="col-lg-8">
                            <div class="row">
                                <div class="col-md-11">
                                  <input id="dropper" type="text" readonly class="form-control">
                                </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                            <div class="row">
                                <div class="col-md-11">
                                  <input id="pengirim" type="text" readonly class="form-control">
                                </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <?php foreach ($transfer as $value){
                      if (!array_key_exists($value->no_kwitansi,$pembayaran)){
                        if(isset($value->pengembang)){
                          $pengembang = $karyawan[$value->pengembang];
                        }else{
                          $pengembang = "";
                        }

                        if(isset($value->leader)){
                          $leaders = $karyawan[$value->leader];
                        }else{
                          $leaders = "";
                        }
                       ?>
                      <tr onclick="pilihbarang('{{$value->id}}','{{$value->no_kwitansi}}','{{$value->tanggal_proses}}','{{$value->id_konsumen}}','{{$konsumen[$value->id_konsumen]['nama']}}',
                                                '{{$konsumen[$value->id_konsumen]['alamat']}}','{{$konsumen[$value->id_konsumen]['no_hp']}}','{{$gudang[$value->id_gudang]}}','{{$karyawan[$value->sales]}}',
                                                '{{$pengembang}}','{{$leaders}}','<?php if (isset($karyawan[$value->manager])){ echo $karyawan[$value->manager]; }else{ echo ""; } ?>',
                                                '{{$user[$value->admin_p]}}',
                                                '<?php if (isset($kategori[$value->kategori])){ echo $kategori[$value->kategori]; }else{ echo ""; } ?>',
                                                '<?php if (isset($karyawan[$value->qc])){ echo $karyawan[$value->qc]; }else{ echo ""; } ?>',
                                                '<?php if (isset($karyawan[$value->dropper])){ echo $karyawan[$value->dropper]; }else{ echo "";} ?>',
                                                '<?php if (isset($karyawan[$value->pengirim])){ echo $karyawan[$value->pengirim]; }else{ echo "";} ?>',
                                                '<?php if (isset($user[$value->admin_g])){ echo str_replace("'","",$user[$value->admin_g]);; }else{ echo "";} ?>',
                                                '<?php if (isset($user[$value->admin_v])){ echo str_replace("'","",$user[$value->admin_v]); }else{ echo "";} ?>',
                                                '{{$value->manager}}','{{$value->leader}}','{{$value->ongkos_kirim}}','{{$konsumen[$value->id_konsumen]['kategori_konsumen']}}')">
    ...
    function pilihbarang(id,no_kwitansi,tanggal_proses,id_konsumen,nama_pemilik,alamat,no_hp,nama_gudang,sales,
      pengembang,leader,manager,admin_p,kategori,qc,dropper,pengirim,admin_g,admin_v,id_manager,id_leader,ongkos_kirim,kategori_konsumen){
      $('#transfer').modal('hide');
         ...
         success: function(response){
           ...
           if (Number(response['barang'][i]['terkirim']) > 0) {
             ...
             if(kategori_konsumen == "PKP"){
                 var ppn = Math.round(11/111 * Number(response['barang'][i]['harga_jual']));
                 cell7.innerHTML = numberWithCommas(ppn);
             }else{
                 var ppn = 0;
                 cell7.innerHTML = "";
             }
             
             cell8.innerHTML = numberWithCommas(Number(response['barang'][i]['harga_jual'])-Number(ppn));
             var pot = Number(response['barang'][i]['potongan']) + (Number(response['barang'][i]['sub_potongan']) / Number(response['barang'][i]['terkirim']));
             cell9.innerHTML = numberWithCommas(pot);
             cell10.innerHTML = numberWithCommas(response['barang'][i]['sub_total']);
