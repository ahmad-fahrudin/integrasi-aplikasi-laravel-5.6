...
@section('content')
              ...
              <div class="col-md-12">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">
                      <nav aria-label="breadcrumb">
                          <ol class="breadcrumb ">
                              <li class="breadcrumb-item text-muted" aria-current="page">Data Transaksi Pengembangan</li>
                          </ol>
                      </nav>
                    </h4>
                    <?php if (Auth::user()->level == "1" || Auth::user()->level == "4"): ?>
                    <div class="col-md-4">
                      <form action="{{url('prosespengembangan')}}" method="post">
                  ...
                  <table id="insentif_table" class="table table-striped table-bordered no-wrap" style="width:100%">
                      ...
                      <tbody>
                        <?php $no=1; $jumlah = 0; foreach ($pengembangan as $key => $value):?>
                          <tr>
                            <td>
                            ...
                            <td align="right">
                              <?php if ($value->jenis == "Trip" || $value->jenis == "Revisi Penambahan Insentif" || $value->jenis == "Bagi Hasil HPP" || $value->jenis == "Fee Transfer Insentif"): $jumlah += $value->jumlah;?>
                                {{ribuan($value->jumlah)}}
                              <?php endif; ?>
                            </td>
                            <td align="right">
    ...
    function pilihbarang(id,nama,nik,alamat,hp,no,ats,nama_bank,saldo){
      ...
      $.ajax({
             url: 'ceksaldopengembang/'+id,
             type: 'get',
             dataType: 'json',
             async: false,
             success: function(response){
                document.getElementById("saldo").value = numberWithCommas(response['saldo']);
             }
           });
