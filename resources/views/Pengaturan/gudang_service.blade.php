...
@section('content')
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">
                      <nav aria-label="breadcrumb">
                          <ol class="breadcrumb ">
                              <li class="breadcrumb-item text-muted" aria-current="page">Pengaturan > Produksi > <a href="https://stokis.app/?s=Gudang+Service+Produksi" target="_blank">Gudang Service</a></li>
                          </ol>
                      </nav>
                    </h4>
                    <hr>
                    ...
                    <div class="row" hidden>
                      <div class="col-md-6">
                        <form action="{{url('insertgudangservice')}}" method="post" enctype="multipart/form-data">
                          {{csrf_field()}}
                        <div class="row">
                          <label class="col-lg-6"><strong>Tambah Gudang Service</strong></label>
                        </div>
                        <br>
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                      <input required type="text" name="nama" class="form-control" placeholder="Ketik nama gudang baru...">
                                  </div>
                              </div>
                          </div>
                        </div>
                        <br>
                        <div class="row">
                          <label class="col-lg-3">Kepala Gudang</label>
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                      <input required type="text" class="form-control" name="kepala_gudang" id="nama_kepala" placeholder="Ketik dan pilih nama kepala gudang baru..." onkeyup="showResult(this.value)">
                                      <input type="hidden" class="form-control" id="id_kepala" name="karyawan">
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                    <select name="kabupaten" class="form-control" id="kotas" onchange="CKabupaten('kecamatans')">
                        ...
                        <div class="row">
                          ...
                          <div class="col-lg-8">
                              <div class="row">
                                  <div class="col-md-11">
                                      <input required name="no_hp" type="text" class="form-control" placeholder="Ketik Nomor Telepon cabang baru...">
                                  </div>
                              </div>
                          </div>
                        </div>
                  ...
                  <label><strong><h3>Data Gudang Service</h3></strong></label>
                      ...
                      <tbody>
                        <?php $no=1;foreach ($gudang as $value) {  ?>
                          <tr>
                              ...
                              <td><?php if(isset($data_kecamatan[$value->kecamatan])){ echo $data_kecamatan[$value->kecamatan]; }; ?></td>
                              <td><?php if(isset($data_kabupaten[$value->kabupaten])){ echo $data_kabupaten[$value->kabupaten]; }; ?></td>
                              <td><?php if(isset($data_provinsi[$value->provinsi])){ echo $data_provinsi[$value->provinsi]; }; ?></td>
                              <td>
                                <button class="btn btn-default" onclick="edit('{{$value->id}}')"><i class="icon-pencil"></i></button>
                                <button hidden class="btn btn-default" onclick="Deleted('{{$value->nama}}','{{$value->id}}')"><i class="icon-trash"></i></button>
                              </td>
                          </tr>
                         <?php $no++;} ?>
                      </tbody>
    ...
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form action="{{url('updategudangservice')}}" method="post" enctype="multipart/form-data">
          {{csrf_field()}}
          <input type="hidden" name="id" id="id">
          Nama Gudang:
          <input required id="nama" name="nama" type="text" class="form-control">
          Kepala Gudang:
          <input required id="kepala_gudang" name="kepala_gudang" type="text" class="form-control" onkeyup="showResults2(this.value)">
          <input type="hidden" class="form-control" id="id_kepala2" name="karyawan">
          <div id="livesearch2"></div>
          Provinsi:
          <select required id="provinsiss" name="provinsi" class="form-control" onchange="CProvinsis('kotass')">
          ...
          Kabupaten / Kota:
          <select name="kabupaten" class="form-control" id="kotass" onchange="CKabupatens('kecamatanss')">
            <option>Pilih Kabupaten/Kota</option>
          </select>
          Kecamatan:
    ...
    {
        $.ajax({
           url: 'editGudangservice/'+value,
           type: 'get',
           dataType: 'json',
           async: false,
           success: function(response){
               document.getElementById("id").value = response['gudang'][0]['id'];
               document.getElementById("nama").value = response['gudang'][0]['nama'];
    ...
    function Deleted(name,value)
    {
      Swal.fire(
        ...
        if (result.value) {
          location.href="{{url('/deletegudangservice/')}}"+"/"+value;
        }
      });
    }
       ...
       success: function(response){
         ...
         if (response.length > 0) {
           for (var i = 0; i < response.length; i++) {
             ...
             document.getElementById("livesearch").innerHTML += "<button class='btn btn-default' type='button' onclick='PilihOrg("+temp+")'>"+response[i]['nama']+"</button>"+"<br>";
           }
         }else{
           document.getElementById("livesearch").innerHTML = "";
         }
    ...
    $.ajax({
       ...
       success: function(response){
         ...
         if (response.length > 0) {
           for (var i = 0; i < response.length; i++) {
             var temp = response[i]['id']+","+'"'+response[i]['nama']+'"';
             document.getElementById("livesearch2").innerHTML += "<button class='btn btn-default' type='button' onclick='PilihOrg2("+temp+")'>"+response[i]['nama']+"</button>"+"<br>";
           }
         }else{
           document.getElementById("livesearch2").innerHTML = "";
         }
